...
export function BottomNav() {
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-bg/90 backdrop-blur-md border-t border-white/5">
        ...
        {navItems.map((item) => (
          ...
          >
            {({ isActive }) => (
              <>
                <span
                  className={`w-8 h-1 rounded-full mx-auto mb-1 transition-all duration-200 ${
                    isActive ? 'bg-purple-500' : 'bg-transparent'
                  }`}
                />
                <span className="transition-all duration-200">{item.icon(isActive)}</span>
                <span className={`text-[10px] ${isActive ? 'text-purple-400 font-medium' : 'font-medium'}`}>
                  {item.label}
                </span>
              </>
            )}
